...
const requestThroughCommandSpan = Effect.gen(function* () {
...
describe('layer', () => {
  it.effect('omits trace propagation headers by default', () =>
    Effect.gen(function* () {
      const { fetch, requestHeaders } = makeFakeFetch()

      yield* requestThroughCommandSpan.pipe(
  ...
  it.effect(
    'plain FetchHttpClient.layer propagates trace headers inside a span',
    () =>
      Effect.gen(function* () {
        const { fetch, requestHeaders } = makeFakeFetch()
