  ...
  return h.div(
    ...
    [
      ...
      para(
        ...
        ': Effect’s Fetch-backed client with trace header propagation disabled, so the tracing span the runtime wraps around every Command doesn’t leak ',
        inlineCode('traceparent'),
        ' headers onto requests and trigger CORS preflights. Tests can swap it for a mock.',
      ),
      infoCallout(
