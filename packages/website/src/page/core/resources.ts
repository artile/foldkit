  ...
  return h.div(
    ...
    [
      ...
      para(
        ...
        ' inside its Effect. Both placements work, so the question is which one a given service should use. The deciding lens: is the service an ambient app-wide capability with one correct configuration, or a per-Command decision where different Commands want different implementations? Four criteria sharpen the call.',
      ),
      bullets(
        h.span(
          [],
      ...
      para(
        ...
        ' from ',
        inlineCode('foldkit/http'),
        ' is Effect’s Fetch-backed client with trace header propagation disabled by default: the runtime runs every Command inside a tracing span, and without that default the client would add ',
        inlineCode('traceparent'),
        ' headers that trigger CORS preflights against plain APIs and dev proxies. ',
        inlineCode('KeyValueStore'),
